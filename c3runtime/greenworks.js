...
if (process.platform == "darwin") {
  ...
  try {
    ...
    console.log("*** ERR *** require('./greenworks-osx'): " + e);

    throw new Error(
      "greenworks-osx64 not loaded. Missing file or incorrect binary version?"
    );
  }
  ...
  if (process.arch == "x64") {
    try {
      ...
      console.log("*** ERR *** require('./greenworks-win64': " + e);

      throw new Error(
        "greenworks-win64 not loaded. Missing file or incorrect binary version?"
      );
    }
  } else if (process.arch == "ia32") {
    try {
      ...
      console.log("*** ERR *** require('./greenworks-win32': " + e);

      throw new Error(
        "greenworks-win32 not loaded. Missing file or incorrect binary version?"
      );
    }
  }
...
greenworks.ugcGetItems = function (
  options,
  ugc_matching_type,
  ugc_query_type,
  success_callback,
...
greenworks.ugcSynchronizeItems = function (
  options,
  sync_dir,
  success_callback,
  error_callback
